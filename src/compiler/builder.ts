    ...
    function getResolvedFromCache(
        ...
        if (!decodedReusableCache.getOwnMap().size && decodedReusableCache.redirectsMap.size === 1) {
            if (isArray(reusableCache)) {
                setBuildInfoResolutionEntries(decodedReusableCache, compilerOptions, reusableCache);
            }
            else {
                if (reusableCache.own) setBuildInfoResolutionEntries(decodedReusableCache, compilerOptions, reusableCache.own);
